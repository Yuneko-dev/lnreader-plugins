...
export async function handleVideoPlugin(
  inputDir: string,
  config: any,
  id: string,
  pluginName: string,
  ...
  template = template.replace(
    '/* __VBOOK_CUSTOM_JS__ */',
    `customJS = 'src/multi/vbook_${folderName}/index.js';`,
  );
  ...
  // Video-specific execution logic for chap.js + track.js concurrency
  const chapCode = loadScriptRecursiveRaw(config.script['chap']);
  const trackCode = loadScriptRecursiveRaw('track.js');

  const trackTransformed = transformVBookScript(
    trackCode.replace(/function\s+execute\s*\(/g, 'function trackExecute('),
  );
  const chapTransformed = transformVBookScript(chapCode);
    ...
    await Promise.all((servers || []).map(async (srv: any) => {
        try {
            ...
            if (trackResult && trackResult.data) {
                validTracks.push({
                    title: srv.title || srv.name || 'Server',
                    data: trackResult.data,
                    type: trackResult.type || 'native'
                });
            }
        } catch(e) {
            console.error('Lỗi khi lấy track cho server', srv.title || srv.name, e);
        }
    }));
    
    if (validTracks.length === 0) throw new Error("Không lấy được đường dẫn video cho bất kỳ server nào.");
    return this.buildPlayerHtml(validTracks);
  `;

  template = template.replace('/* __VBOOK_CHAP__ */', () => chapExecution);
