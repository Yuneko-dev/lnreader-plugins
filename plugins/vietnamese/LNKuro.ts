...
class LNKuroPlugin implements Plugin.PluginBase {
    ...
    async popularNovels(
        ...
        if (url == `${this.site}/`) {
            ...
            $('.truyen-card').each((i, el) => {
                ...
                const url = card.find('.truyen-tooltip .tooltip-footer a.tooltip-button').attr('href')!;
                if (url) {
    ...
    async parseNovel(novelPath: string): Promise<Plugin.SourceNovel> {
        ...
        webnovelSection.each((i, el) => {
            ...
            w('.novel_kuro ul.chapter-list_kuro li').each((i, el) => {
                ...
                const chapterName = aTag.text().trim().replace(/[\n\s]+/g, " ");
                const chapterUrl = aTag.attr('href')!;
                const releaseTime = li.find('span.date').text().trim() || li.find('span').text().trim(); // DD/MM/YYYY
    ...
    async parseChapter(chapterPath: string): Promise<string> {
        ...
        chapterElementRaw.children('div').filter(function () {
